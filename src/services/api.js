...
export async function loginAndGetUser(email, password) {
  ...
  if (!profile || !profile.id) {
    try { profile = await authAPI.getMe(); }
    catch {
      profile = {
...
export const projectsAPI = {
  ...
  create: (name, nodes, edges)    => post('/projects/', { name, nodes: nodes.map(nodeToBackend), edges: edges.map(edgeToBackend) }),
  update: (id, name, nodes, edges)=> put(`/projects/${id}`, { name, nodes: nodes.map(nodeToBackend), edges: edges.map(edgeToBackend) }),
...
export const pathsAPI = {
  shortest: (start, end, algorithm = 'dijkstra') =>
    post('/paths/shortest', { start, end, algorithm: algoToBackend(algorithm) }).then(pathResultToFrontend),
  best: (start, end) =>
    post('/paths/best', { start, end, algorithm: 'dijkstra' }).then(pathResultToFrontend),
};

export const fleetAPI = {
  getVehicles:   ()                             => get('/fleet/vehicles'),
  getStats:      ()                             => get('/fleet/stats'),
  updateVehicle: (id, status, speed, location) => patch(`/fleet/vehicles/${id}`, { status, speed, location }),
};

export const parkingAPI = {
  getLots: () => get('/parking/lots').then(d => {
    const list = d?.lots ?? d;
